...
def load_contract_abi_and_address(path):
    ...
    try:
        logger.debug("Loading contract ABI from path: %s", path)
        if not os.path.exists(path):
            raise FileNotFoundError(f"Contract ABI file not found at path: {path}")
        with open(path, 'r') as file:
            contract_data = json.load(file)
        logger.info("Contract ABI loaded successfully from: %s", path)
        return contract_data
    except FileNotFoundError:
        logger.error("Contract ABI file not found at path: %s", path)
    except json.JSONDecodeError as e:
        logger.error("Error decoding JSON from the contract ABI file at %s: %s", path, e)
    except Exception as e:
        logger.error("Unexpected error loading contract ABI from %s: %s", path, e, exc_info=True)
    return None
...
def getLastUnsuccessfulBCObject(web3, contract_abi, contract_address, last_known_block):
    ...
    """
    logger.debug("Fetching last unsuccessful blockchain object starting from block: %s", last_known_block)
    try:
        contract = web3.eth.contract(address=contract_address, abi=contract_abi)
        event_filter = contract.events.AuctionSale.create_filter(from_block=int(last_known_block))
        events = event_filter.get_all_entries()
        logger.info("Number of events fetched: %d", len(events))

        if len(events) >= 2:
            logger.info("Multiple events found, processing...")

        for event in events:
            mintID = event["args"]["mintID"]
            jobState = dbFunctions.read_value_from_column('jobState', mintID)
            logger.debug("Event mintID: %s, JobState in database: %s", mintID, jobState)
            if jobState != "done":
                logger.info("Found an event that is not done, returning event data")
                return Web3.to_json(event)

        logger.info("No suitable event found that meets the criteria")
    except Exception as e:
        logger.error("Error occurred while fetching blockchain object: %s", e, exc_info=True)
    return None

def convert_ipfs_url(url):
    logger.debug("Converting IPFS URL: %s", url)
    if url.startswith("https://aqua-few-camel-178.mypinata.cloud/ipfs/"):
        converted_url = "ipfs://" + url.split("/")[-1]
        logger.info("Converted IPFS URL: %s", converted_url)
        return converted_url
    else:
        logger.error("URL does not match the expected IPFS format: %s", url)
...
def create_ipfsjson(name, character, obstacle, surface, picIPFS, vidIPFS, glbIPFS):
    logger.debug("Creating IPFS JSON for: %s", name)
    try:
        ...
        logger.info("IPFS JSON created successfully at: %s", json_file_path)
        return json_file_path
    except Exception as e:
        logger.error("Failed to create IPFS JSON for %s: %s", name, e, exc_info=True)
        raise

def pinContentToIPFS(firstUnsuccess: Dict[str, Any], pinata_api_key: str, pinata_secret: str) -> Dict[str, Any]:
    logger.debug("Uploading content to IPFS for: %s", firstUnsuccess.get('fullname'))
    HEADERS = {
    ...
    for ext in extensions:
        ...
        logger.debug("Checking existence of file: %s", file_path)
        ...
        try:
            with open(file_path, 'rb') as fp:
                ...
                logger.info("File %s uploaded successfully, response: %s", file_path, response_data)
                ...
                if ipfs_hash:
                    ...
                    logger.error("IPFS hash not found in the response for file: %s", file_path)
                    raise Exception("IPFS hash not found in response")
        except requests.exceptions.RequestException as e:
            logger.error("HTTP request failed for %s: %s", file_path, e, exc_info=True)
            raise Exception(f"HTTP request failed for {file_path}: {e}")
        except Exception as e:
            logger.error("An error occurred while uploading %s to IPFS: %s", file_path, e, exc_info=True)
            raise

def uploadJsonToIPFS(firstUnsuccess: Dict[str, Any], pinata_api_key: str, pinata_secret: str) -> Dict[str, Any]:
    logger.debug("Uploading JSON to IPFS for: %s", firstUnsuccess.get('fullname'))
    HEADERS = {
        "pinata_api_key": pinata_api_key,
        "pinata_secret_api_key": pinata_secret,
    }
    ...
    if not json_file_path.exists():
        logger.error("The JSON file '%s' does not exist.", json_file_path)
    ...
    try:
        with open(json_file_path, 'rb') as fp:
            ...
            response.raise_for_status()
            response_data = response.json()
            logger.info("JSON file uploaded successfully, response: %s", response_data)
            ...
            if ipfs_hash:
                ...
                logger.error("IPFS hash not found in the response for JSON file: %s", json_file_path)
                raise KeyError("IPFS hash not found in response")
    except requests.exceptions.RequestException as e:
        logger.error("HTTP request failed for %s: %s", json_file_path, e, exc_info=True)
        raise Exception(f"HTTP request failed for {json_file_path}: {e}")
    except Exception as e:
        logger.error("An error occurred while uploading JSON %s to IPFS: %s", json_file_path, e, exc_info=True)
        raise

def mint(tokenURI, to_address, contract_address, owner_private_key, owner_address, provider_url, mintID):
    logger.debug("Minting NFT with tokenURI: %s to address: %s", tokenURI, to_address)
    try:
        ...
        logger.debug("Estimated gas cost in ETH: %s", estimated_gas_cost_eth)
        ...
        logger.info("Transaction hash: %s", web3.to_hex(tx_hash))
        tx_receipt = web3.eth.wait_for_transaction_receipt(tx_hash)
        if tx_receipt.status == 1:
            logger.info("Transaction succeeded with hash: %s", web3.to_hex(tx_hash))
        else:
            logger.warning("Transaction failed with hash: %s", web3.to_hex(tx_hash))
        ...
        logger.info("Transaction successful with hash: %s", web3.to_hex(tx_hash))
        return tx_receipt
    except Exception as e:
        logger.error("Failed to mint NFT: %s", e, exc_info=True)
        raise Exception(e)
